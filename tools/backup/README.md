...
3. ページ右下に「AxnosPaint データ復元」パネルが表示されるので、
   **「.axp を選んで復元」ボタンをクリック**。
   - ブラウザの仕様上、ファイル選択ダイアログはユーザーのクリック操作が
     必要なため、コンソールから直接開くことはできません。
4. ファイル選択ダイアログで `.axp` を選ぶ。
5. 「IndexedDB を上書きします」確認ダイアログで OK。
6. 「復元完了」のメッセージが出たら **必ずページをリロード**。
...
- **「File chooser dialog can only be shown with a user activation」と表示される**:
  ブラウザはコンソール実行から直接ファイル選択ダイアログを開けません。
  本スクリプトはこの制限を回避するため、ページ右下にボタンを表示します。
  そのボタンをクリックしてからファイルを選んでください。
